...
export default function ConvocatoriasPage() {
  ...
  // Preselección desde el Calendario global: /client/convocatorias?proyecto=<id>
  useEffect(() => {
    api.get<any>("/projects").then(r => {
      const list = Array.isArray(r) ? r : (r?.data ?? []);
      setProjects(list);
      const params = new URLSearchParams(window.location.search);
      const preProyecto = params.get("proyecto");
      if (preProyecto && list.some((p: any) => p.id === preProyecto)) setSelectedProject(preProyecto);
    }).catch(console.error);
  ...
  const aprobar = async () => {
    setBusy(true);
    try {
      await api.post(`/projects/${selectedProject}/aprobar`, { comentario: "Aprobado desde convocatorias" });
