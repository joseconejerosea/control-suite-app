...
const SEV_STYLE: Record<string, { background: string; color: string }> = {
  critica: { background: "rgba(79,70,229,0.15)", color: "var(--danger)" },
      ...
      <div className="animate-fade-up" style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 64px)" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <h1 style={{ fontSize: 20, fontWeight: 700, display: "flex", alignItems: "center", gap: 8 }}>
            <Brain size={20} style={{ color: "var(--primary)" }} /> Control Mind
          </h1>
        ...
        {dashboard && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
            {[
              ...
              { label: "Aprobadas hoy", value: dashboard.aprobadas_hoy ?? 0, color: "var(--success)" },
              { label: "Ahorro estimado", value: dashboard.ahorro_estimado ? `$${Number(dashboard.ahorro_estimado).toLocaleString("es-CL")}` : "—", color: "var(--primary)" },
              { label: "Proyectos monitoreados", value: dashboard.proyectos_activos ?? "—", color: "var(--primary)" },
          ...
          {tabs.map(t => {
            ...
            return (
              <button key={t.key} onClick={() => setTab(t.key as any)}
                style={{ padding: "9px 16px", fontSize: 13, fontWeight: tab === t.key ? 600 : 400, borderBottom: tab === t.key ? "2px solid var(--primary)" : "2px solid transparent", color: tab === t.key ? "var(--foreground)" : "var(--muted-foreground)", background: "none", cursor: "pointer", marginBottom: -1, display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                            ...
                            {p.estado !== "abierta" && <span style={{ fontSize: 11, fontWeight: 600, color: p.estado === "aprobada" ? "var(--success)" : "#94a3b8", textTransform: "capitalize" }}>{p.estado}</span>}
                          </div>
                          ...
                          {p.impacto_estimado && <div style={{ fontSize: 12, color: "var(--primary)", marginTop: 6 }}>Impacto estimado: {p.impacto_estimado}</div>}
                        </div>
                        {p.estado === "abierta" && (
                          <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                            <button onClick={() => accionPropuesta(p.id, "aprobar")}
                              style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: "color-mix(in srgb, var(--success) 15%, transparent)", color: "var(--success)", cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
                              <Check size={12} /> Aprobar
                            </button>
                            <button onClick={() => accionPropuesta(p.id, "rechazar")}
                              style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: "color-mix(in srgb, var(--danger) 10%, transparent)", color: "var(--danger)", cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
                              <X size={12} /> Rechazar
                            </button>
                          </div>
                        )}
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              ...
              <button onClick={() => sendMessage()} disabled={!input.trim() || sending}
                style={{ padding: "10px 20px", borderRadius: 12, border: "none", background: input.trim() ? "var(--primary)" : "var(--secondary)", color: input.trim() ? "#fff" : "var(--muted-foreground)", cursor: input.trim() ? "pointer" : "not-allowed", fontWeight: 600, fontSize: 13 }}>
                Enviar
              </button>
            </div>
          </div>
            ...
            {!loadingAnalista && (insights || resumen) && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                {resumen && (
                  <div className="rounded-xl border p-5" style={{ background: "var(--card)", borderColor: "var(--border)" }}>
                    <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
                      <BarChart2 size={16} style={{ color: "var(--primary)" }} /> Resumen mensual
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 16 }}>
                      {[
                        { label: "Activaciones", value: resumen.total_activaciones ?? "—", color: "var(--primary)" },
                        { label: "Checkins totales", value: resumen.total_checkins ?? "—", color: "var(--success)" },
                        { label: "Incidencias", value: resumen.total_incidencias ?? "—", color: "var(--danger)" },
                      ].map(({ label, value, color }) => (
                    ...
                    {resumen.analisis_ia && (
                      <div style={{ padding: "12px 16px", borderRadius: 10, background: "rgba(79,70,229,0.08)", border: "1px solid rgba(79,70,229,0.2)" }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: "var(--primary)", marginBottom: 6, textTransform: "uppercase" }}>Analisis IA</div>
                ...
                {insights && (
                  <div className="rounded-xl border p-5" style={{ background: "var(--card)", borderColor: "var(--border)" }}>
                    <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
                      <Brain size={16} style={{ color: "var(--primary)" }} /> Insights operacionales
                    </div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {insights.insights.map((ins: any, i: number) => (
                          <div key={i} style={{ padding: "12px 16px", borderRadius: 10, background: "var(--secondary)", display: "flex", gap: 12, alignItems: "flex-start" }}>
                            <div style={{ width: 8, height: 8, borderRadius: "50%", background: ins.tipo === "alerta" ? "var(--danger)" : ins.tipo === "oportunidad" ? "var(--success)" : "var(--primary)", marginTop: 4, flexShrink: 0 }} />
                            <div>
