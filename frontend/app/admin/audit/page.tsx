...
export default function AuditPage() {
  ...
  const summaryKpis = [
    { label: "Total AI Cost", value: fmtUSD(data?.total_cost_usd), color: "#6366f1" },
    { label: "Claude calls", value: data?.total_calls ?? "—", color: "#f59e0b" },
    { label: "Tokens usados", value: data?.total_tokens ? `${(data.total_tokens / 1000).toFixed(1)}K` : "—", color: "#34b96e" },
    { label: "Avg per call", value: fmtUSD(data?.avg_cost_per_call), color: "#60a5fa" },
  ];
          ...
          {data?.by_client?.length > 0 && (
            <div className="rounded-xl border mb-5" style={{ background: "var(--card)", borderColor: "var(--border)" }}>
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  {data.by_client.map((c: any, i: number) => (
                    <tr key={i} style={{ borderTop: "1px solid var(--border)" }}>
                      <td className="px-4 py-3 text-sm font-medium">{c.client_name ?? c.client_id?.slice(0, 8)}</td>
                      <td className="px-4 py-3 text-sm">{c.calls ?? "—"}</td>
                      <td className="px-4 py-3 text-sm">{c.tokens ? `${(c.tokens / 1000).toFixed(1)}K` : "—"}</td>
                      <td className="px-4 py-3 text-sm font-semibold">{fmtUSD(c.cost_usd)}</td>
