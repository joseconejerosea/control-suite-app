...
**Actualización (2026-06-23):** H5 ✅, H6 ✅, H7 ✅, H10 ✅, H11 ✅. **TODOS los ALTOS cerrados salvo H8** (throttle+tope costo Mind — DIFERIDO, espera números de negocio). Siguen ABIERTOS: H8 + MEDIOS (M1,M2,M4,M5,M6,M7,M8,M9,M12) + BAJOS.

**Tests de regresión (paso 4 del plan)** — 4 suites e2e nuevas, **36 tests verdes**, sin DB:
- `test/prompt-shield.e2e-spec.ts` (H7) — 14 tests: inyección EN/ES, encoded, sensibles, longitud, sanitize, `checkWithAI` fail-closed.
- `test/webhook-verify-token.e2e-spec.ts` (H10) — `constantTimeEqual` + fail-closed de ambos endpoints de verificación.
- `test/document-upload-limits.e2e-spec.ts` (H11) — MIME 415, truncado 413, target_table 400, allow-list OK.
- `test/f5-validation.e2e-spec.ts` (H5/H6) — `persona_id` del body → 400, emails inválidos → 400, `cerrarActivacion` idempotente → 409.
...
| H5 ✅ | `modules/f5/f5.controller.ts:31,48,84,118,145` | Bodies tipados `any`; insertan `body.*` sin validar. `body.persona_id` permite impersonar checkins; `destinatarios`/`htmlReporte` sin validar → email/HTML injection. | DTOs validados en `modules/f5/dto/f5.dto.ts` (class-validator, `ValidationPipe` global whitelist). `createCheckin` ya NO acepta `persona_id` del body → siempre `user.sub`. `destinatarios` validado `@IsEmail({each})`; `htmlReporte` `@IsString @MaxLength`. |
| H6 ✅ | `modules/f5/f5.controller.ts:144` | `cerrarActivacion` setea `status='completed'`/`estado_f5='cerrada'` desde cualquier estado, sin precondición ni idempotencia. | UPDATE con precondición `AND estado_f5 IS DISTINCT FROM 'cerrada' AND status <> 'cancelled'`; si no matchea fila → `ConflictException` (409). No re-cierra. |
| H7 ✅ | `common/ai/prompt-shield.service.ts:10-18,69` | 7 regex en inglés; injection en español/encoded pasa. `checkWithAI` *fail-open*. Mind solo usa `checkLocal`; `mind-analista`, `cron`, `report.processor`, `invoices` (OCR) no aplican shield. | **Shield unificado y endurecido**: patrones EN+ES (voseo/tuteo) + encoded (base64/escapes/atob) + datos sensibles + `sanitized`. `checkWithAI` ahora **fail-closed** (bloquea ante error de API o formato inesperado). **Alcance (decisión 2026-06-23): solo entrypoints de USUARIO** → Mind chat (`checkLocal` endurecido, sin clasificador IA por costo — ver H8) y **WhatsApp text** (`handleText` ahora gatea con el shield). Contenido ingerido (OCR/facturas/cron) NO se shieldea a propósito (falsos positivos + costo). **Duplicado muerto `modules/prompt-shield/` ELIMINADO** (no lo usaba nadie). |
...
| H10 ✅ | `modules/webhooks/webhooks.controller.ts:37` y `whatsapp.webhook.controller.ts:39` | Comparación de verify-token con `===` (no constant-time) y fallback al literal `'change_me'` si falta el env. | Helper `common/utils/constant-time.ts` (`timingSafeEqual` con guarda de longitud); ambos endpoints de verificación ahora **fail-closed** si falta `WEBHOOK_SECRET`/`WHATSAPP_VERIFY_TOKEN` (sin fallback a literal) y comparan en tiempo constante. |
| H11 ✅ | `modules/document-ingestion/document-ingestion.controller.ts:41-67` | Upload sin límite de tamaño (buffer completo en memoria) ni allow-list de MIME. `target_table` viene del multipart sin validar → elige tabla destino. | El límite `fileSize: 10MB` ya estaba en `@fastify/multipart` (main.ts) pero truncaba en silencio → ahora rechaza con `PayloadTooLargeException` si `file.truncated`. **MIME allow-list** (PDF/JPG/PNG/Excel/CSV/Word) → `UnsupportedMediaTypeException`. `target_table` validado contra el enum `TargetTable` → `BadRequestException`. (Decisión 2026-06-23: PPT/MP4 EXCLUIDOS de este endpoint.) |
...
| M8 ✅ | `modules/onboarding/onboarding.service.ts` | `verifyWhatsAppOtp` nunca validaba el `code`; seteaba `is_active=true` siempre (verificación de fachada). | RESUELTO — al pasar a número Meta único se eliminó todo el provisioning por tenant (`provisionWhatsApp`/`verifyWhatsAppOtp`). El canal WhatsApp se activa al configurarse; no hay OTP de fachada. |
| M9 ✅ | `modules/onboarding/onboarding.service.ts` | `phone_number_id` hardcodeado como fallback. | RESUELTO — el provisioning por tenant se eliminó (número global); ya no hay `phone_number_id` por canal. |
